...
def sample_task_te(tasks, way, shot, DIM, CHN=1, shuffle_nc=None):  # for testing
    """
    sample a task from tasks for ProtoNet
    :param DIM:
    :param CHN:
    ...
    support, query = support.to(device), query.to(device)
    return support, query, shuffle_nc


def semi_sample_task(tasks, un_task, way, shot, n_u, DIM, CHN=1, shuffle_nc=None):
    # assert tasks.shape[1] >= shot * 2
    n_s = n_q = shot
    tasks = torch.from_numpy(tasks)
    tasks = tasks.reshape(tasks.shape[0], tasks.shape[1], CHN, DIM)
    un_task = torch.from_numpy(un_task)
    un_task = un_task.reshape(un_task.shape[0], un_task.shape[1], CHN, DIM)

    if shuffle_nc is None:
        shuffle_nc = torch.randperm(tasks.shape[0])[:way]  # training
    support = torch.zeros([way, n_s, CHN, DIM])
    query = torch.zeros([way, n_q, CHN, DIM])
    unlabel = torch.zeros([way, n_u, CHN, DIM])

    for i, cls in enumerate(shuffle_nc):
        selected = torch.randperm(tasks.shape[1])[:n_s + n_q]  # 0:10
        support[i] = tasks[cls, selected[:n_s]]
        query[i] = tasks[cls, selected[n_s:n_s + n_q]]
        selected = torch.randperm(un_task.shape[1])[:n_u]  # 0:5
        unlabel[i] = un_task[cls, selected]
    support, query, unlabel = support.to(device), query.to(device), unlabel.to(device)
    return support.float(), query.float(), unlabel.float(), shuffle_nc


def semi_sample_task_te(tasks, un_task, way, shot, n_u, DIM, CHN=1, shuffle_nc=None):  # for testing
    n_s = n_q = shot
    tasks = torch.from_numpy(tasks)
    tasks = tasks.reshape(tasks.shape[0], tasks.shape[1], CHN, DIM)
    un_task = torch.from_numpy(un_task)
    ...
    for i, cls in enumerate(shuffle_nc):
        support[i] = tasks[cls, :n_s]
        selected = torch.randperm(tasks.shape[1])[:n_q]
        query[i] = tasks[cls, selected]
        # selected = torch.randperm(un_task.shape[1])[:n_u]
        # unlabel[i] = un_task[cls, selected[:n_u]]
        unlabel[i] = un_task[cls, :n_u]
    support, query, unlabel = support.to(device), query.to(device), unlabel.to(device)
    return support.float(), query.float(), unlabel.float(), shuffle_nc
